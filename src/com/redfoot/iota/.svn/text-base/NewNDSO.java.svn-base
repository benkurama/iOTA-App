...
public class NewNDSO extends Activity implements OnItemClickListener {
 // =========================================================================
 // TODO Variables
 // =========================================================================
	private static String SAVED_BARANGAY = "";
	//private static String NDSOMAP = "";
	// --->
	private static String NDSOMAP001 = "";
	private static String NDSOMAP002 = "";
	// --<
	private MainApp m_maApp;
	private BroadcastReceiver m_brSMS;
	private ListView m_lvEstablishments;
 // =========================================================================
 // TODO Overrides / Life Cycle
 // =========================================================================
    @Override
    public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.newndso);
					...
					if (sNum.length() == 0) {
						...
						if (MainApp.DEBUG == 0) {
							if (m_maApp.getGPSStatus() != LocationProvider.AVAILABLE) {
								Utility.alertNoGPS(NewNDSO.this);
							} else {
								sendNDSOMAP(sNum);
							}
						} else {
			...
			case MainApp.DIALOG_IOTA_NUM_ID:
				...
				bOK.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						...
						if (sNum.length() == 0) {
							bValid = false;
				        	new AlertDialog.Builder(NewNDSO.this)
	...
	protected void onRestoreInstanceState(Bundle savedInstanceState) {
		...
		NewNDSO.SAVED_BARANGAY = savedInstanceState.getString(this.getString(R.string.Barangay));
	}
	// ---------------------------------------------------------------------
	@Override
 ...
 // TODO Main Functions
 // =========================================================================
	protected void sendNDSOMAP(String sNum) {
		...
		/*NDSOMAP001 = this.getString(R.string.NDSOMAP) + " " + etName.getEditableText().toString() + "/" + sSubtypeCode + "/" + etNo.getEditableText().toString()
		...
		int textID = MainApp.getTextID(this);
		
		NDSOMAP001 = this.getString(R.string.NDSOMAP) + "A " + textID + " " + etName.getEditableText().toString() + "/" + sSubtypeCode + "/" + etNo.getEditableText().toString()
				  + "," + etStreet.getEditableText().toString() + "," + ((CodeDescPair)sBarangay.getSelectedItem()).Code + "," + ((CodeDescPair)sCity.getSelectedItem()).Code
				  + "/" + m_maApp.getGPSLong() + "/" +  m_maApp.getGPSLat() + "/" + sStoreSizeCode + "/" + sumList + "/" + sCbMerchPresentString + "/" + sCbMerchDominantString
				  + "/" + stockGP.getEditableText().toString() + "," + stockTM.getEditableText().toString() + "," + stockSm.getEditableText().toString() + ","
				  + stockTnT.getEditableText().toString() + "," + stockSun.getEditableText().toString() + "/" + aveReloadGP.getEditableText().toString() + "," + aveReloadTM.getEditableText().toString() + "," 
		...
		NDSOMAP002 = this.getString(R.string.NDSOMAP) + "B " + textID + " /" + etMobtel.getEditableText().toString() + "/" + aveSimGP.getEditableText().toString() + "," + aveSimTM.getEditableText().toString() + "," + aveSimSm.getEditableText().toString() + "," + 
				  aveSimTnT.getEditableText().toString() + "," + aveSimSun.getEditableText().toString() + "/" + sInterviewCode + "/" + sInterviewName.getEditableText().toString() 
				  + "/" + sPhone.getEditableText().toString() + "/" + sOwnerName.getEditableText().toString() + "/" + etRemarks.getEditableText().toString();
		...
		PendingIntent pi = PendingIntent.getBroadcast(NewNDSO.this, MainApp.REQUEST_ACTION_SENT,  new Intent(this.getPackageName() + MainApp.INTENT_ACTION_SENT), 0);
		SmsManager sms = SmsManager.getDefault();
		
		//region Sending a Multiple Message Commented
		/*ArrayList<String> parts = sms.divideMessage(NDSOMAP001);
		...
		*/
		...
		// reserved for future text message
		
		sms.sendTextMessage(sNum, null, NDSOMAP001, null, null);
		sms.sendTextMessage(sNum, null, NDSOMAP002, pi, null);
	}
	// ---------------------------------------------------------------------
	protected boolean validateFields() {
		...
		String sMobtel = ((EditText)this.findViewById(R.id.d_etMobtel)).getEditableText().toString().toString();
	...
	protected void fillBarangays(String code) {
    	...
    	for (CodeDescPair cdp : alBarangays) {
    		aaAdapter.add(cdp);
    		if (cdp.Code.equals(NewNDSO.SAVED_BARANGAY)) {
    	...
    	if (NewNDSO.SAVED_BARANGAY.length() > 0) {
			sBarangays.setSelection(iIndex, true);
    	} else {
    		sBarangays.setSelection(iLast, true);
    	}
    	NewNDSO.SAVED_BARANGAY = ((CodeDescPair)sBarangays.getSelectedItem()).Code;
	}
	// ---------------------------------------------------------------------
    	...
    	if (intent.getAction().equals(this.getPackageName() + MainApp.INTENT_ACTION_SENT)) {
	    	switch (resultCode) {
	    		case Activity.RESULT_OK:
	    			...
	    			cvTemp.put("body", NDSOMAP001 + NDSOMAP002);
    		...
    		if (Utility.getDialogResult()) {
    			this.sendNDSOMAP(m_maApp.getDatabase().getIotaNum());
    		}
    	}
	}
 // =========================================================================
